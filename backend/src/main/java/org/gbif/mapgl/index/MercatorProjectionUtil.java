...
public class MercatorProjectionUtil {
   ...
   */
  public static int getOffsetX(double lat, double lng, int zoom, int tileSize) {
   ...
   */
  public static int getOffsetY(double lat, double lng, int zoom, int tileSize) {
    double y = toNormalisedPixelCoords(lat, lng).getY();
    int scale = 1 << zoom;
    y *= scale * tileSize;
    int tileY = toTileY(lat, zoom);
    return (int) (y - (tileSize * tileY));
  }
