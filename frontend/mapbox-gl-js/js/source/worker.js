...
var DatacubeTile = require('./datacube_tile_transformer');
...
util.extend(Worker.prototype, {
    ...
    'load geojson tile': function(params, callback) {
        ...
        for (var i = 0; i < 30000; i++){
            test.push({
                geometry: [[Math.round(Math.random()*4096), Math.round(Math.random()*4096)]],
                type: 1,
                tags: {name: Math.round(Math.random()*100)}
            });
        }
        geoJSONTile.features = test;
    ...
    'load datacube tile': function(params, callback) {
        var source = params.source,
            uid = params.uid;
        ...
        function done(err, data) {
            ...
            var dcCube = new DatacubeTile(data);
            var vtBuffer = dcCube.getAsVectorTileBuffer(512);
            tile.data = new vt.VectorTile(new Protobuf(new Uint8Array(vtBuffer)));
            tile.parse(tile.data, this.layers, this.actor, callback);
